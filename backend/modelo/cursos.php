...
class Cursos {
    ...
    public function obtenerMisCursos($id) {
        // var_dump($id);
        $sentencia = "SELECT cursos.id_curso, cursos.titulo, cursos.descripcion, cursos.precio, cursos.duracion, usuarios.nombre FROM cursos, usuarios WHERE cursos.id_profesor = usuarios.id_usuario AND cursos.id_curso in (select id_curso from inscripciones where id_usuario = ?)";
        $consulta = $this->conn->prepare($sentencia);
